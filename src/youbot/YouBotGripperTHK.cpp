...
YouBotGripperTHK::YouBotGripperTHK(const std::string name, const std::string configFilePath)
...
YouBotGripperTHK::~YouBotGripperTHK() {
  // Bouml preserved body begin 00067FF1
        if(ethercatMaster.isThreadActive()){
            for (unsigned int i = 0; i < GRIPPERJOINTS; i++) {
...
void YouBotGripperTHK::doJointCommutation() {
...
void YouBotGripperTHK::calibrateGripperTHK(const bool forceCalibration) {
  // Bouml preserved body begin 000A9C71
    ...
    tempdummy.rpm = 0;
    calibrationVel.assign(GRIPPERJOINTS, tempdummy);
    std::vector<quantity<si::current> > maxCurrent;
    quantity<si::current> tempdummy2;
    maxCurrent.assign(GRIPPERJOINTS, tempdummy2);
    std::vector<bool> doCalibration;
    doCalibration.assign(GRIPPERJOINTS, true);
    ...
    for (unsigned int i = 0; i < GRIPPERJOINTS; i++) {
      ...
      if (direction == "POSITIV") {
        calibrationVel[i].rpm = 20 / gearratio;
    ...
    finished.assign(GRIPPERJOINTS, false);
...
void YouBotGripperTHK::calibrateGripper(const bool forceCalibration) {
...
void YouBotGripperTHK::softClose(float TargetPosition, float fMaxVelocity, float fAbortCurrent){
    try
    {
        JointSensedCurrent sensedCur;
        JointSensedAngle sensedAngle;
        MaximumPositioningVelocity maxVel;
        maxVel.setParameter(fMaxVelocity * radian_per_second); // 6.28* 2 * radian_per_second
        joints[0].setConfigurationParameter(maxVel);
        JointAngleSetpoint setAngle;
        setAngle.angle=TargetPosition * radian;
        joints[0].setData(setAngle);
        sleep(1);
        while(1)
        {
            joints[0].getData(sensedCur);
            joints[0].getData(sensedAngle);
            if(abs(sensedCur.current.value())>fAbortCurrent)
            {
                //fAbortCurrent=0.5 on default
                std::cout << "Current is over "<< fAbortCurrent <<" Ampere, thats why stop the Gripper" << endl;
                SLEEP_MILLISEC(50);
                break;
            }
            if(abs(sensedAngle.angle.value()-setAngle.angle.value())<0.05 ){
                std::cout << "Position reached, thats why stop the Gripper" << endl;
                SLEEP_MILLISEC(50);
                break;
            }
        }
        ...
        joints[0].setData(setAngle);
        SLEEP_MILLISEC(100);
        joints[0].restoreConfigurationParameter(maxVel);
        SLEEP_MILLISEC(100);
    }
 ...
 void YouBotGripperTHK::softCloseThread(float TargetPosition, float fMaxVelocity, float fAbortCurrent){


     boost::thread startThread(boost::bind(&YouBotGripperTHK::softClose,this,TargetPosition, fMaxVelocity,fAbortCurrent));
 }


///return a joint form the arm1
///@param armJointNumber 1-5 for the arm joints
YouBotJoint& YouBotGripperTHK::getArmJoint(const unsigned int armJointNumber) {
  // Bouml preserved body begin 0004F7F1

    if (armJointNumber <= 0 || armJointNumber > GRIPPERJOINTS) {
...
YouBotGripper& YouBotGripperTHK::getArmGripper() {
...
void YouBotGripperTHK::setJointData(const std::vector<JointAngleSetpoint>& JointData) {
  // Bouml preserved body begin 0008FDF1
    if (JointData.size() != GRIPPERJOINTS)
...
void YouBotGripperTHK::getJointData(std::vector<JointSensedAngle>& data) {
  // Bouml preserved body begin 0008FE71
    data.resize(GRIPPERJOINTS);
...
void YouBotGripperTHK::setJointData(const std::vector<JointVelocitySetpoint>& JointData) {
...
void YouBotGripperTHK::getJointData(std::vector<JointSensedVelocity>& data) {
...
void YouBotGripperTHK::setJointData(const std::vector<JointCurrentSetpoint>& JointData) {
...
void YouBotGripperTHK::getJointData(std::vector<JointSensedCurrent>& data) {
...
void YouBotGripperTHK::setJointData(const std::vector<JointTorqueSetpoint>& JointData) {
...
void YouBotGripperTHK::getJointData(std::vector<JointSensedTorque>& data) {
...
void YouBotGripperTHK::commutationFirmware200() {
  // Bouml preserved body begin 0010D8F1
    ...
    isCommutated.assign(GRIPPERJOINTS, false);
    unsigned int u = 0;
    ...
    for (unsigned int i = 1; i <= GRIPPERJOINTS; i++) {
...
void YouBotGripperTHK::commutationFirmware148() {
...
void YouBotGripperTHK::initializeJoints() {
 ...
 //   LOG(info) << "Initializing Joints";
    ...
    if (noSlaves < GRIPPERJOINTS) {
      throw std::runtime_error("Not enough ethercat slaves were found to create a YouBotGripperTHK!");
    }
